...
void loop(void)
{
    ...
    while ((!terminate || i < 100) && !done) {
        ...
        for (j = 0; j < num_inst; j++) {
            ...
            status = mpr_sig_get_inst_status(recvsig1, id, 1);
            ...
            status = mpr_sig_get_inst_status(sendsig1, id, 1);
            ...
            status = mpr_sig_get_inst_status(recvsig2, id, 1);
            if (status & MPR_STATUS_UPDATE_REM) {
            ...
            status = mpr_sig_get_inst_status(sendsig2, id, 1);
            if (status & MPR_STATUS_REL_DNSTRM) {
                eprintf("%s.%"PR_MPR_ID" got downstream release\n",
