...
int main(int argc, char **argv)
{
    ...
    types = mpr_msg_atom_get_types(atom);
    if (!mpr_type_get_is_str(types[0])) {
        eprintf("1: Type error retrieving @host property.");
        result = 1;
        goto done;
    }
    if (mpr_msg_atom_get_len(atom) != 1) {
        eprintf("1: Length error retrieving @host property.");
        result = 1;
        goto done;
    }
    vals = mpr_msg_atom_get_values(atom);
    if (strcmp(&(*vals)->s, "127.0.0.1")!=0)
        result |= 1;
    eprintf("1: @host = \"%s\" %s\n", &(*vals)->s,
    ...
    if (mpr_msg_atom_get_len(atom) != 4) {
        eprintf("1: Length error retrieving @src@min property.");
        result = 1;
        goto done;
    }
    vals = mpr_msg_atom_get_values(atom);
    for (i = 0; i < mpr_msg_atom_get_len(atom); i++) {
        if (vals[i]->f != r[i])
            result = 1;
        eprintf("1: @src@min[%d] = %f %s\n", i, vals[i]->f, result ? "WRONG" : "(correct)");
        if (result)
            goto done;
    }
    ...
    if (mpr_msg_get_num_atoms(msg) != 3) {
        eprintf("3: Wrong number of atoms.\n");
        result = 1;
        goto done;
    }

    atom = mpr_msg_get_atom(msg, 0);
    if (strcmp(mpr_msg_atom_get_key(atom), "foo")) {
        eprintf("3: Could not get -@foo property.\n");
        result = 1;
        goto done;
    }
    if (!(mpr_msg_atom_get_prop(atom) & PROP_REMOVE)) {
    ...
    atom = mpr_msg_get_atom(msg, 2);
    if (strcmp(mpr_msg_atom_get_key(atom), "bar")) {
