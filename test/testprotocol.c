...
mapper_protocol protocol = MAPPER_PROTO_UDP;
...
void set_map_protocol(mapper_protocol proto)
{
    ...
    protocol = proto;
    eprintf("Sending %s\n", proto == MAPPER_PROTO_UDP ? "UDP" : "TCP");
    if (!verbose)
        printf("\n");
}
...
void loop()
{
    ...
    while ((!terminate || i < 50) && !done) {
        ...
        if (!verbose) {
            printf("\r  %s  Sent: %4i, Received: %4i   ",
                   protocol == MAPPER_PROTO_UDP ? "UDP" : "TCP", sent, received);
            fflush(stdout);
        }
    }
}
    ...
    for (i = 1; i < argc; i++) {
        if (argv[i] && argv[i][0] == '-') {
            ...
            for (j = 1; j < len; j++) {
                switch (argv[i][j]) {
                    case 'h':
                        printf("testprotocol.c: possible arguments "
