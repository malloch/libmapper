...
static void mpr_net_add_dev_methods(mpr_net net, mpr_local_dev dev)
{
    ...
    for (i = 0; i < NUM_DEV_HANDLERS_GENERIC; i++) {
        for (j = 0; j < NUM_NET_SERVERS; j++)
            lo_server_add_method((net)->servers[j],
    ...
    if (net->num_devs == 0) {
        ...
        for (i = 0; i < NUM_DEV_HANDLERS_GENERIC; i++) {
            ...
            if (found)
                continue;
            for (j = 0; j < NUM_NET_SERVERS; j++)
                lo_server_del_method((net)->servers[j],
                                     net_msg_strings[dev_handlers_generic[i].str_idx],
                                     dev_handlers_generic[i].types);
        }
    }
...
void mpr_net_send(mpr_net net)
{
    ...
    if (BUNDLE_DST_SUBSCRIBERS == net->addr.dst) {
        ...
        lo_send_bundle_from(net->addr.bus, net->servers[SERVER_MESH_UDP], net->bundle);
    else {
        int serv = (LO_TCP == lo_address_get_protocol(net->addr.dst)) ? SERVER_MESH_TCP : SERVER_MESH_UDP;
        lo_send_bundle_from(net->addr.dst, net->servers[serv], net->bundle);
    }
...
void mpr_net_free(mpr_net net)
{
    ...
    FUNC_IF(lo_server_free, net->servers[SERVER_MESH_UDP]);
    FUNC_IF(lo_server_free, net->servers[SERVER_MESH_TCP]);
...
static void mpr_net_maybe_send_ping(mpr_net net, int force)
{
    ...
    if (now.sec > net->next_sub_ping) {
        ...
        if (now.sec < net->next_bus_ping) {
            for (i = 0; i < net->num_devs; i++) {
                mpr_local_dev dev = net->devs[i];
                if (dev->subscribers) {
                    trace("  sending sync to subscribers\n");
                    mpr_net_use_subscribers(net, dev, MPR_DEV);
    ...
    trace("  sending sync to bus\n");
    mpr_net_use_bus(net);
    for (i = 0; i < net->num_devs; i++) {
        if (net->devs[i]->registered)
            mpr_net_send_device_sync(net, net->devs[i]);
    }
    ...
    while (list) {
        ...
        if (elapsed > TIMEOUT_SEC) {
            if (clk->rcvd.msg_id > 0) {
                ...
                if (num_maps) {
                    trace_dev(lnk->local_dev, "Removing link to unresponsive device '%s' "
        ...
        if (num_maps && mpr_obj_get_prop_as_str(&lnk->remote_dev->obj, MPR_PROP_HOST, 0)) {
            ...
            if (lnk->remote_dev->addr_alt)
                lo_send_bundle_from(lnk->remote_dev->addr_alt, net->servers[SERVER_MESH_TCP], bun);
            if (LO_UDP == lo_address_get_protocol(lnk->remote_dev->addr))
                lo_send_bundle_from(lnk->remote_dev->addr, net->servers[SERVER_MESH_UDP], bun);
            else
                lo_send_bundle_from(lnk->remote_dev->addr, net->servers[SERVER_MESH_TCP], bun);
                       ...
                       lo_message msg, void *user)
{
    ...
    if (graph->autosub || mpr_graph_subscribed_by_dev(graph, name)) {
        ...
        if (a && !remote->addr && !remote->addr_alt) {
            char* url = lo_address_get_url(a);
            remote->addr = lo_address_new_from_url(url);
            url[4] = 't';
            url[5] = 'c';
            remote->addr_alt = lo_address_new_from_url(url);
            free(url);
        }
    ...
    TRACE_RETURN_UNLESS(i == net->num_devs, 0, "  ignoring /device message from self\n");
            ...
            if (MPR_DIR_OUT == rs->slots[i]->dir) {
                ...
                while (cpy) {
                    ...
                    if (mpr_link_get_is_local(link) && map->dst->link == link) {
                        mpr_net_use_mesh(net, link->remote_dev->addr);
                           ...
                           int ac, lo_message msg, void *user)
{
    ...
    if (mpr_sig_set_from_msg(sig, props)) {
        if (dev->subscribers) {
            ...
            trace_dev(dev, "informing subscribers (SIGNAL) (1)\n");
...
void mpr_net_handle_map(mpr_net net, mpr_local_map map, mpr_msg props)
{
    ...
    if (map->is_local_only && map->expr) {
        ...
        if (dev->subscribers) {
            ...
            trace_dev(dev, "informing subscribers (SIGNAL) (2)\n")
            mpr_net_use_subscribers(net, dev, MPR_SIG);
            for (i = 0; i < map->num_src; i++)
                mpr_sig_send_state(map->src[i]->sig, MSG_SIG);
            mpr_sig_send_state(map->dst->sig, MSG_SIG);

            trace_dev(dev, "informing subscribers (MAPPED) (1)\n")
    ...
    for (i = 0; i < map->num_src; i++) {
        ...
        if (!map->src[i]->link || !map->src[i]->link->remote_dev->addr) {
        ...
        if (MPR_STATUS_READY == map->status) {
            ...
            for (i = 0; i < map->num_src; i++) {
                if (map->src[i]->sig->is_local) {
                    ...
                    inform_device_subscribers(net, dev);
                    trace_dev(dev, "informing subscribers (SIGNAL) (3)\n");
                    mpr_net_use_subscribers(net, dev, MPR_SIG);
                    mpr_sig_send_state(map->src[i]->sig, MSG_SIG);
                }
            }
            if (map->dst->sig->is_local) {
                ...
                inform_device_subscribers(net, dev);
                trace_dev(dev, "informing subscribers (SIGNAL) (4)\n");
                mpr_net_use_subscribers(net, dev, MPR_SIG);
                mpr_sig_send_state(map->dst->sig, MSG_SIG);
            }
        }
        ...
        if (map->is_local) {
            ...
            for (i = 0; i < map->num_src; i++) {
                if (map->src[i]->sig->is_local) {
                    mpr_local_dev dev = (mpr_local_dev)map->src[i]->sig->dev;
                    if (dev->subscribers) {
                        trace_dev(dev, "informing subscribers (MAPPED) (2)\n")
            ...
            if (map->dst->sig->is_local) {
                mpr_local_dev dev = (mpr_local_dev)map->dst->sig->dev;
                if (dev->subscribers) {
                    trace_dev(dev, "informing subscribers (MAPPED) (3)\n")
                           ...
                           int ac, lo_message msg, void *user)
{
    ...
    if (MPR_LOC_DST == loc && !map->dst->sig->is_local) {
        trace("  forwarding /map/modify to remote destination.\n");
        if (map->dst->sig->dev->addr)
            mpr_net_use_mesh(net, map->dst->sig->dev->addr);
        else
            mpr_net_use_bus(net);
        mpr_net_add_msg(net, path, 0, lo_message_clone(msg));
        mpr_net_send(net);
        goto done;
    }
    else if (MPR_LOC_SRC == loc && !map->src[0]->sig->is_local) {
        trace("  forwarding /map/modify to remote source.\n");
        if (map->src[0]->sig->dev->addr)
            mpr_net_use_mesh(net, map->src[0]->sig->dev->addr);
        else
            mpr_net_use_bus(net);
        mpr_net_add_msg(net, path, 0, lo_message_clone(msg));
        mpr_net_send(net);
    ...
    if (updated) {
        ...
        for (i = 0; i < map->num_src; i++) {
            if (map->src[i]->rsig) {
                mpr_local_dev dev = (mpr_local_dev)map->src[i]->sig->dev;
                if (dev->subscribers) {
                    trace_dev(dev, "  informing subscribers (MAPPED) (4)\n")
                    mpr_net_use_subscribers(net, dev, MPR_MAP_OUT);
                    mpr_map_send_state((mpr_map)map, -1, MSG_MAPPED);
                }
            }
        ...
        if (map->dst->rsig) {
            mpr_local_dev dev = (mpr_local_dev)map->dst->sig->dev;
            if (dev->subscribers) {
                trace_dev(dev, "  informing subscribers (MAPPED) (5)\n")
                mpr_net_use_subscribers(net, dev, MPR_MAP_IN);
                mpr_map_send_state((mpr_map)map, -1, MSG_MAPPED);
            }
        }
                         ...
                         int ac, lo_message msg, void *user)
{
    ...
    if (!map->dst->is_local || !map->dst->rsig) {
        if (map->dst->link && map->dst->link->remote_dev->addr) {
            mpr_net_use_mesh(net, map->dst->link->remote_dev->addr);
        ...
        for (i = 0; i < map->num_src; i++) {
            ...
            if (src->rsig || !src->link || !src->link->remote_dev->addr)
                continue;
            mpr_net_use_mesh(net, src->link->remote_dev->addr);
    ...
    for (i = 0; i < map->num_src; i++) {
        if (map->src[i]->sig->is_local) {
            ...
            trace_dev(dev, "informing subscribers (SIGNAL) (5)\n");
            mpr_net_use_subscribers(net, dev, MPR_SIG);
            mpr_sig_send_state(map->src[i]->sig, MSG_SIG);

            trace_dev(dev, "informing subscribers (UNMAPPED) (1)\n")
            mpr_net_use_subscribers(net, dev, MPR_MAP_OUT);
    ...
    if (map->dst->sig->is_local) {
        ...
        trace_dev(dev, "informing subscribers (SIGNAL) (6)\n");
        mpr_net_use_subscribers(net, dev, MPR_SIG);
        mpr_sig_send_state(map->dst->sig, MSG_SIG);

        trace_dev(dev, "informing subscribers (UNMAPPED) (2)\n")
        mpr_net_use_subscribers(net, dev, MPR_MAP_IN);
        mpr_map_send_state((mpr_map)map, -1, MSG_UNMAPPED);
    }
        ...
        if (dev->addr && dev->addr_alt) {
            if (LO_TCP == lo_address_get_protocol(lo_message_get_source(msg))) {
                ...
                if (dev->subscribed) {
                    ...
                    mpr_net_use_mesh(net, dev->addr);
                    mpr_graph_send_subscribe_msg(graph, dev, 0, 0);
                }

                lo_address_free(dev->addr);
                dev->addr = tcp;
            }
        }
        if (!dev->subscribed && graph->autosub) {
            trace_graph("requesting metadata for device '%s'.\n", &av[0]->s);
            mpr_graph_subscribe(graph, dev, MPR_DEV, 0);
        }
    }
