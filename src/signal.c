...
static int initialize_instance(mpr_local_sig lsig, mpr_sig_inst si, mpr_id_pair ids, int activate);
...
static mpr_sig_inst _find_inst_by_id(mpr_local_sig lsig, mpr_id id)
{
    ...
    sip = &si;
    si.id = id;
    sipp = bsearch(&sip, lsig->inst, lsig->obj.num_inst, sizeof(mpr_sig_inst), _compare_inst_ids);
        ...
        *locked = 1;
        for (i = 0; i < sig->num_maps_in; i++) {
            ...
            if (tmp_ids->global == ids->global) {
                mpr_id_map_decref_global(mpr_obj_get_id_map((mpr_obj)sig->dev), ids);
    ...
    *locked = 1;
    for (i = 0; i < sig->num_maps_out; i++) {
        ...
        for (; id_map_idx < sig->id_map_size; id_map_idx++) {
            ...
            if (!si && (all || mpr_obj_get_use_inst((mpr_obj)sig)))
                continue;
            inst_idx = si->idx;
            mpr_local_map_set_updated(map, inst_idx);
            if (!all)
...
again:
    ...
    if (global_id) {
        mpr_id_map dev_id_map = mpr_obj_get_id_map((mpr_obj)dev);
        /* don't activate an instance just to release it again */
        remote_ids = mpr_id_map_get_global(dev_id_map, global_id);
        ...
        if (sig->id_map[id_map_idx].status & RELEASED_LOCALLY) {
            ...
            if (0 == vals) {
                ...
                mpr_id_map_decref_global(dev_id_map, sig->id_map[id_map_idx].ids);
                sig->id_map[id_map_idx].ids = 0;
                if (remote_ids) {
                    mpr_id_map_decref_global(dev_id_map, remote_ids);
                }
            }
    ...
    if (vals == 0) {
        if (global_id) {
            ...
            mpr_id_map_decref_global(dev_id_map, ids);
            if (remote_ids) {
                mpr_id_map_decref_global(dev_id_map, remote_ids);
            }
        }
...
static int activate_instance(mpr_local_sig lsig, mpr_id *local_id, mpr_id *global_id)
{
    int i;
    mpr_sig_inst si;
    mpr_id_map dev_id_map = mpr_obj_get_id_map((mpr_obj)lsig->dev);
    ...
    if (local_id) {
        ...
        ids = mpr_id_map_get_local(dev_id_map, *local_id);
        ...
        ids = mpr_id_map_get_global(mpr_obj_get_id_map((mpr_obj)lsig->dev), *global_id);
        if (ids) {
            trace("found existing id_pair for global id\n");
            remote_ids = mpr_id_map_get_local(dev_id_map, *global_id);
    ...
    for (i = 0; i < lsig->id_map_size; i++) {
        if (!lsig->id_map[i].ids && (si = lsig->id_map[i].inst)) {
            if (local_id) {
                if (si->id != *local_id)
                    continue;
            }
            else {
                ids = mpr_id_map_get_local(dev_id_map, si->id);
    ...
    for (i = 0; i < lsig->obj.num_inst; i++) {
        si = lsig->inst[i];
        if (   (!lsig->obj.ephemeral || !(si->status & MPR_STATUS_ACTIVE))
            && (local_id || !mpr_id_map_get_local(dev_id_map, si->id))) {
    ...
    for (i = 0; i < lsig->id_map_size; i++) {
        ...
        if ((si = lsig->id_map[i].inst)) {
            ...
            if (ids && global_id) {
                ...
                mpr_id_pair indirect = mpr_id_map_add(dev_id_map, ids->global, *global_id, 1);
    ...
    {
        mpr_id last = 0;
        while ((ids = mpr_id_map_get_global_free(dev_id_map, last))) {
    ...
    if (local_id) {
        ...
        qsort(lsig->inst, lsig->obj.num_inst, sizeof(mpr_sig_inst), _compare_inst_ids);
    }
    if (!ids) {
        /* Claim id map locally */
        ids = mpr_id_map_add(dev_id_map, si->id,
    ...
    return initialize_instance(lsig, si, ids, 1);
}
                            ...
                            int flags, uint8_t activate, uint8_t call_handler)
{
    ...
    for (i = 0; i < lsig->id_map_size; i++) {
        mpr_sig_id_pair sip = &lsig->id_map[i];
        if (!sip->ids)
            continue;
...
static int _reserve_inst(mpr_local_sig lsig, mpr_id *id, void *data)
{
    ...
    lsig->inst = realloc(lsig->inst, sizeof(mpr_sig_inst) * (lsig->obj.num_inst + 1));
    lsig->inst[lsig->obj.num_inst] = (mpr_sig_inst) calloc(1, sizeof(struct _mpr_sig_inst));
    si = lsig->inst[lsig->obj.num_inst];
...
static void mpr_sig_release_inst_internal(mpr_local_sig lsig, int id_map_idx)
{
    ...
    process_maps(lsig, id_map_idx);
    if (smap->ids && mpr_id_map_decref_local(mpr_obj_get_id_map((mpr_obj)lsig->dev), smap->ids)) {
        smap->ids = 0;
    }
...
void mpr_local_sig_release_inst_by_origin(mpr_local_sig lsig, mpr_dev origin)
{
    int i;
    mpr_id id;
    mpr_id_map dev_id_map;
    mpr_time time;

    RETURN_UNLESS(lsig->obj.ephemeral);

    mpr_time_set(&time, MPR_NOW);
    dev_id_map = mpr_obj_get_id_map((mpr_obj)lsig->dev);
...
void mpr_sig_remove_inst(mpr_sig sig, mpr_id id)
{
    ...
    --lsig->obj.num_inst;
    lsig->inst = realloc(lsig->inst, sizeof(mpr_sig_inst) * lsig->obj.num_inst);
